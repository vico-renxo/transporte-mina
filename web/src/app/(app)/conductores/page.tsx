...
function FormConductor({ cond, onSave, onClose }: { cond?: Conductor; onSave: () => void; onClose: () => void }) {
  ...
  const handleSave = async () => {
    ...
    if (!cond && !password) { toast.error('La contraseña es obligatoria para nuevos conductores'); return; }
    setSaving(true);
    try {
  ...
  const fields = [
    ...
    ['Contraseña' + (cond ? ' (dejar vacío = sin cambio)' : ' *'), password, setPassword, 'password'],
    ['N° Licencia *', licencia, setLicencia, 'text'],
    ['Teléfono', telefono, setTelefono, 'tel'],
  ];
          ...
          return (
            <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition-colors">
              ...
              <div className="mt-3 pt-3 border-t border-slate-800 grid grid-cols-2 gap-2 text-xs">
                ...
                <div><p className="text-slate-500">Teléfono</p><p className="text-slate-300 font-semibold">{c.telefono || '—'}</p></div>
